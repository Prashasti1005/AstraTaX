...
const Chatbot = () => {
  ...
  const GOOGLE_AI_API_KEY = "AIzaSyAR-mYCaE7sEq1Rli--3r8940gEWjfg26E"; // Replace with your actual API key
  ...
  const handleAsk = async () => {
    ...
    try {
      ...
      const botResponse = data.candidates?.[0]?.content?.parts?.[0]?.text || "⚠️ No response.";
  ...
  return (
    <div className="flex items-center justify-center h-screen bg-gradient-to-br from-gray-900 to-black text-white p-6">
      <div className="max-w-3xl w-full h-[90vh] bg-gray-950/60 backdrop-blur-2xl border border-gray-700 shadow-2xl rounded-3xl flex flex-col overflow-hidden">
        ...
        <div
          ...
          className="flex-1 overflow-y-auto p-6 space-y-4 custom-scrollbar bg-gray-900/30 backdrop-blur-lg"
        >
          {messages.map((msg, index) => (
            <div
              key={index}
              className={`flex ${msg.sender === "user" ? "justify-end" : "justify-start"} items-center`}
            >
              {msg.sender === "bot" && <FaRobot className="text-yellow-400 text-2xl self-end mr-3" />}
              <div
                className={`p-4 rounded-2xl max-w-xs text-lg shadow-lg transition-transform transform hover:scale-105 ${
                  msg.sender === "user"
        ...
        <div className="p-5 border-t border-gray-700 flex items-center space-x-4 bg-gray-900/80 rounded-b-3xl shadow-lg">
          ...
          <button
            onClick={handleAsk}
            className="bg-gradient-to-r from-yellow-500 to-yellow-600 text-black px-6 py-3 rounded-xl hover:scale-110 transition-transform duration-300 shadow-lg"
            disabled={loading}
          >
            <FaPaperPlane />
          </button>
